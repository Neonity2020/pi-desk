          ...
          {selectedProject && <div className="composer-area"><div className="composer">
            ...
            <div className="composer-bottom">
              ...
              <div className="composer-tools composer-tools-right">
                <div className="composer-menu-wrap" ref={modelMenuRef}><button className="model-control" onClick={toggleModelMenu}><span>{currentModel?.name || 'Pi 模型'}</span><span className="reasoning-label">{levelLabel(thinkingLevel)} 推理</span><ChevronDown size={13}/></button>{modelMenu && <div className="composer-popover model-popover"><div className="popover-label">Pi 模型 · 提供商</div><label className="model-search"><Search size={14}/><input autoFocus value={modelSearch} onChange={event => setModelSearch(event.target.value)} placeholder="搜索模型或提供商"/><kbd>⌘F</kbd></label>{modelError && <div className="model-error">{modelError}</div>}{models.length === 0 && !modelError && <div className="model-loading">正在读取 Pi 模型…</div>}{(() => { const query = modelSearch.trim().toLowerCase(); const filtered = models.filter(item => `${item.name} ${item.provider} ${item.id}`.toLowerCase().includes(query)); if (!filtered.length) return models.length > 0 ? <div className="model-loading">没有匹配的模型</div> : null; const groups = new Map<string, PiModel[]>(); for (const item of filtered) { const list = groups.get(item.provider) || []; list.push(item); groups.set(item.provider, list) } return [...groups.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([provider, items]) => { const searching = query.length > 0; const collapsed = searching ? false : (collapsedProviders[provider] ?? provider !== currentModel?.provider); return <div className="model-group" key={provider}><button className="model-group-head" onClick={() => !searching && setCollapsedProviders(old => ({ ...old, [provider]: !(old[provider] ?? provider !== currentModel?.provider) }))}><ChevronRight size={12} className={collapsed ? '' : 'open'}/><span>{provider}</span><small>{items.length}</small></button>{!collapsed && items.map(item => <button key={`${item.provider}/${item.id}`} className={currentModel?.provider === item.provider && currentModel.id === item.id ? 'selected' : ''} onClick={async () => { if (!selectedTask) return; try { const result = await window.desk.setPiModel(selectedTask.id, item.provider, item.id); const cached = modelCache.get(selectedTask.id); if (cached) modelCache.set(selectedTask.id, { ...cached, current: result.model, thinkingLevel: result.thinkingLevel, thinkingLevels: result.thinkingLevels }); setCurrentModel(result.model); setThinkingLevels(result.thinkingLevels); setThinkingLevel(result.thinkingLevel); setModelMenu(false) } catch (error) { setModelError(String(error)) } }}><span className="model-option"><strong>{item.name}</strong><small>{item.id}</small></span>{currentModel?.provider === item.provider && currentModel.id === item.id && <Check size={13}/>}</button>)}</div> }) })()}{thinkingLevels.length > 0 && <><div className="popover-label">推理强度</div>{thinkingLevels.map(level => <button key={level} className={thinkingLevel === level ? 'selected' : ''} onClick={async () => { if (!selectedTask) return; try { await window.desk.setPiThinkingLevel(selectedTask.id, level); setThinkingLevel(level); const cached = modelCache.get(selectedTask.id); if (cached) modelCache.set(selectedTask.id, { ...cached, thinkingLevel: level }) } catch (error) { setModelError(String(error)) } }}>{levelLabel(level)}{thinkingLevel === level && <Check size={13}/>}</button>)}</>}</div>}</div>
